...
//     <html lang="en">
//       <body
//        className={` ${inter.variable} ${(pathname?.includes("/dashboard") || pathname?.includes("/auth")) ? " bg-[#FFF]  " : " !bg-[#3B3B3B] "} !overflow-hidden "`}
//        >
//         <Provider>
//           <Theme>
...
export default function RootLayout({
  ...
  // Dynamically set the class name for body after the component is mounted
  const [className, setClassName] = useState('')

  useEffect(() => {
    const baseClassName = `${inter.variable} !overflow-hidden`
    if (pathname?.includes("/dashboard") || pathname?.includes("/auth")) {
      setClassName(`${baseClassName} bg-[#FFF]`)
    } else {
      setClassName(`${baseClassName} !bg-[#3B3B3B]`)
    }
  }, [pathname])
        ...
        <Provider>
          <Theme>
            <div className={`w-screen h-screen ${pathname?.includes("/dashboard") || pathname?.includes("/auth") ? "" : " !bg-[#3B3B3B] "} overflow-x-hidden overflow-y-hidden relative top-0 bottom-0 !z-[1000000000]`}>
              ...
              <div className={`w-full flex flex-col items-center text-white h-screen overflow-x-hidden relative overflow-y-auto z-30`}>
                <div className={`w-screen lg:h-fit ${pathname?.includes("/dashboard") ? "hidden" : "block"} flex justify-center pb-3`}>
                  <Navbar pathname={pathname}/>
                </div>
                <div className={`w-full h-auto ${!(pathname?.includes("/dashboard") || pathname?.includes("/auth")) ? "px-0" : ""}`}>
                  {children}
                </div>
                {(!pathname?.includes("/auth") && !pathname?.includes("/dashboard")) && (
                  <div className='w-full h-fit mt-auto'>
