...
function Comments(props: Props) {
  ...
  const { isLoading, refetch, isRefetching } = useQuery(
    ["comment"],
    () => actionService.getservicedata(`/content/comments/${id}`),
    {
      onError: (error: any) => {
        console.error(error);
      },
        ...
        <div className=" pb-6 border-b border-[#828282] w-full flex gap-[25px] ">
          ...
          <div className=" pt-2 w-full ">
            <div className=" flex gap-3 items-center ">
              <CustomText className=" leading-[24px] font-medium ">
                {props?.name}{" "}
                <span style={{ fontSize: "13px", color: "#707070" }}>
                  {formatTimeAgo(new Date(props?.created_at ?? "").getTime())}
                </span>
              </CustomText>
              <CustomText className=" leading-[22px] text-[#707070] text-sm "></CustomText>
            </div>
            <CustomText className=" leading-[24px] mt-3 ">
              {props?.comment}
            </CustomText>
          </div>
  ...
  const submit = async () => {
   ...
   // console.log('formik?.errors?.name',formik?.errors?.name ==="required", formik.errors.name, formik.touched.name)
    ...
    addCommentsMutation.mutateAsync(formdata).catch(() => {
      toast({
        title: "Something went wrong",
        status: "error",
        duration: 3000,
      ...
      <div className=" w-full mt-4 px-[25px] py-[21px] flex items-center justify-between border border-[#828282] rounded-[14px] ">
        <div className=" w-full flex items-center gap-[25px]  ">
          ...
          <input
            name="comment"
            onChange={formik.handleChange}
            onFocus={() => formik.setFieldTouched("comment", true, true)}
            placeholder="Add a comment"
            className={` bg-transparent  ${
              formik?.errors?.comment ? " placeholder:text-[#E84545]  " : " "
            }  w-full px-4 outline-none py-2 `}
          />
        </div>
        ...
        >
          <>
            {data?.map((item: CommentData, index: number) => {
              return <CommentData key={index} {...item} />;
            })}
          </>
        </LoadingAnimation>
      </div>
