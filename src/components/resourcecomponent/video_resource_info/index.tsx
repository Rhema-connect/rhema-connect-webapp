...
function VideoResourceInfo(props: Props) {
  ...
  const { isLoading } = useQuery(
    ["videolist", id],
    () =>
      actionService.getservicedata(`/content/${id}`, {
        limit: 20,
        page: 0,
        type: "VIDEO",
      }),
        ...
        <div className=" w-full  flex gap-6  ">
              ...
              {videoError && (
                <div
                  className={` w-full text-lg   ${
                    dashboard ? "bg-white" : "bg-gray-600"
                  } font-semibold h-full justify-center flex items-center `}
                >
          ...
          <div className=" w-fit lg:block hidden  ">
            <div className=" w-[321px] ">
              <CustomText className=" font-bold text-lg leading-7 ">
                Others also likes
              </CustomText>
              <Othervideo dashboard={dashboard} id={id}/>
            </div>
          </div>
        </div>
        <div className=" w-full flex gap-6  ">
