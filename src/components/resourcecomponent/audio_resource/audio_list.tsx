...
function Audiolist(props: Props) {
    ...
    const { results, isLoading, ref, isRefetching } = InfiniteScrollerComponent({ url: `/content?type=AUDIO`, limit: 10, filter: "id", type: "VIDEO", name: "audilist" })
                ...
                <div className=' w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-9 mt-8 ' >
                    {results?.map((item: ContentData, index: number) => {
                        if (index === results?.length - 1) {
                            return (
                                <div role='button' ref={ref} onClick={() => setCurrentData(item)} key={index} className={` ${item?.title === currentData?.title ? " bg-[#BE0027] text-white " : ""} w-full flex items-center shadow-lg py-2 px-4 rounded-xl gap-[18px] `} >
